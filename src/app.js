...
// Memuat data aktivitas dari file bot.js Anda
...
class TitanBot extends Client {
  ...
  async start() {
    try {
      ...
      // Menjalankan sistem rotasi aktivitas setelah bot sukses 'ready'
  ...
  // Fungsi tambahan untuk menangani perputaran aktivitas bot secara aman
  setupPresenceRotation() {
    this.once('ready', () => {
      const pConfig = botConfig?.presence;
      if (!pConfig || !pConfig.activities || pConfig.activities.length === 0) {
        logger.warn('Tidak ada konfigurasi activities yang ditemukan di bot.js.');
        return;
      }

      let currentIndex = 0;
      logger.info('Sistem rotasi status otomatis berhasil dijalankan (Setiap 15 Detik).');

      // Loop penggantian status
      setInterval(() => {
      ...
      }, 15000); // Menggunakan jeda aman 15 detik agar bot Anda terhindar dari pemblokiran Discord API
    });
  }
